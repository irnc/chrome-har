...
export default function entryFromResponse(entry, response, page, options) {
  ...
  /*
   * HAR 1.2 defines `connection` field as:
   *
   * connection [string, optional] (new in 1.2) - Unique ID of the parent
   * TCP/IP connection, can be the client or server port number. Note that a
   * port number doesn't have to be unique identifier in cases where the port
   * is shared for more connections. If the port isn't available for the
   * application, any other unique connection ID can be used instead
   * (e.g. connection index). Leave out this field if the application doesn't
   * support this info.
   *
   * Chrome sets `connection` to the server port number.
   */
  entry.connection = response.connectionId.toString();
  entry._connectionId = response.connectionId.toString();
